...
@Component({
    ...
    <ng-template #delete let-c="close" let-d="dismiss">
        ...
        <div class="modal-footer">
            <button type="button" class="btn btn-danger" (click)="deleteTicket(ticketToCancel);c('')">
                <i class="fa fa-times"></i> Delete Registration
            </button>
                ...
                <div class="col-md-9">
                    ...
                    <div class="ticket-container print-hide" *ngFor="let ticket of myTickets">
                        ...
                        <div class="tc-body">
                            ...
                            <button class="pull-right btn btn-outline-warning" (click)="cancelTicket(delete, ticket);">
                                Cancel registration
                            </button>
                    ...
                    <div class="printable hidden" style="border:2px solid #444;border-radius:5px;
                        margin-bottom:30px;" *ngFor="let ticket of myTickets">
                        <table class="tickets">
                            <tr>
                                    ...
                                    <div class="row">
                                        <div class="col-6">
                                            <div style="padding:1rem">
                                                ...
                                                <h3 style="text-transform: uppercase;padding:0;margin:0;font-size:22px;">
                                                    {{ ticket.ticketTitle }}
                                                    <span *ngIf="ticket.ticketQty > 1">
                                                        ({{ ticket.ticketQty }})
                                                    </span>
                                                </h3>
                                            </div>
...
export class RegistrationsManageComponent implements OnInit {
    ...
    populateTickets() {
        ...
        this.af.list(`/registrations/type/${this.eventId}`).first().subscribe((types) => {
            for (const type of types) {
                console.log(type);
                const path = `/registrations/type/${this.eventId}/${type.$key}`;
                const cancelPath = `/registrations/cancelled/${this.eventId}/${type.$key}`;
    ...
    cancelOrder() {
        ...
        this.af.object(`/registrations/orders/cancelled/users/${this.auth.id}/${this.eventId}`).set(this.registrationWhole).then(() => {
            this.af.object(`/registrations/orders/cancelled/events/${this.auth.id}/${this.eventId}`)
                .set(this.registrationWhole).then(() => {
    ...
    deleteTicket(ticket) {
        ...
        this.af.object(ticket.cancelPath).set(ticket).then(() => {
            this.af.object(ticket.path).remove().then(() => {
                ...
                const ticketRef = this.af.object(`/tickets/live/${this.eventId}/${ticket.ticketRef}`);
                const eventRef = this.af.object(`/events/live/${this.eventId}/data`);

                // UPDATE TICKET COUNTS -- TODO MANAGE THIS IN THE BACKEND LATER
                ticketRef.first().subscribe((ticketData) => {
                    const newTicketsUsedTicket = ticketData.ticketsUsed - ticket.ticketQty;
                    const newTicketsLeftTicket = ticketData.ticketsLeft + ticket.ticketQty;
