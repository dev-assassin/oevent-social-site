...
export class CreateEventService{
    ...
    setNewEvent():Promise<any>{
        return new Promise((resolve, reject)=>{
            let blankEvent:IoEvent = new oEvent();
    ...
    getGeoCode(address): Observable<any>{
        let key = "AIzaSyCXlzLSm5F9D0W16PcX-FQQGi9W51E2SLM";
    ...
    validateTime(groomedDate:GroomedDate,eventType:string):boolean{
        let times = groomedDate.timeString.split("-");
        let minsDifference = (moment(`2012-12-12 ${times[1]}`).toDate().getTime()-moment(`2012-12-12 ${times[0]}`).toDate().getTime())/60000;
    ...
    publish(live:boolean = true){
        ...
        let liveObject$ = this.af.object(`/events/live/${this.eventId}`);
        let profileUpcoming$ = this.af.object(`events/profile/upcoming/${this.auth.id}/${this.eventId}`);
        ...
        this.draftObject$.update({status:status}).then((success)=>{
            ...
            if(live){
                ...
                let preventTwice:boolean = false;
                    ...
                    this.af.object(`/events/components/date/${this.auth.id}/${data.dateKey}`).first().subscribe((dateData)=>{
                        ...
                        if(!preventTwice){
                            ...
                            if(dateData.eventType != "recurring"){
                                ...
                                liveObject$.update({ref: this.eventId, ocode: this.appService.ocode, uid: this.auth.id, data:data}).then((success)=>{
                                ...
                                for(let copy of copies){
                                    ...
                                    this.af.object(`/events/events/${this.auth.id}/${this.id}/childEventKeys/`).remove().then(()=>{
                                        this.af.list(`/events/live/`).push({
                                            ...
                                            this.af.object(`/events/events/${this.auth.id}/${this.id}/childEventKeys/${success.key}`).set({"set":true});
    ...
    // -----------------------------------------------------------------------------------------------------------------------
    onDestroy(){
        for (let reference of this.firebaseConnections){
    ...
    showSavedDraft(){
        let message = (this.isLive())?'Event Saved':'Draft Saved';
    ...
    setErrorInField(field:string,errMsg:string){
        let fieldValidation:FieldValidation = new FieldValidation();
    ...
    setOrganizerPromoter(){
        ...
        this.af.object(`/promoters/events/${this.eventId}/${this.appService.ocode}`).update({datetime:firebase.database.ServerValue.TIMESTAMP}).then(()=>{
