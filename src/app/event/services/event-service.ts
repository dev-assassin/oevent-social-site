...
export class EventService {
    ...
    refOcode:string;
    refOcodeName:string;
    refOcodeSet:boolean = false;
    promoterProfile:IAccountAbout;
    promoterProfileSet:boolean = false;
    ...
    totalTicketQty:number = 0;
    perTicketQty:number[] = [];

    totalUsedTicketQty:number = 0;
    perTicketUsedQty:number = 0;
    ...
    buildTicketRegistration(){
        ...
        if(this.currentTickets){
            for(let ticket of this.currentTickets){
    ...
    groomTicket(ticket:any, index){
        ...
        let newPath = `${this.ticketPath}/${ticket.key}`;
    ...
    addTicketRegistration(ticket:any, ticketType, index, multiRegistrant, buyMultiple){
        for(var i = 0; i < parseInt(this.currentTickets[index].quantity); i++) {
            let newTicket = new TicketRegistration();
    ...
    register(registration:IEventRegistration){
        ...
        this.getPromoterInfoByOcode(registration.ocode).then((promoterAbout)=>{
            ...
            if(this.registrationTickets.length){
                ...
                let childTickets:IEventRegistration[] = [];

                for(let ticket of this.registrationTickets){
                    let childRegistration:IEventRegistration = new EventRegistration();
                ...
                this.registrationUsers().push(registration).then((data)=>{
                    ...
                    this.registrationEvent().set(registration).then((data)=>{
                        ...
                        this.registerNotifications(regNotification).then(()=>{
                            ...
                            for(let ticket of childTickets){
    ...
    clearSingleInput(){
        ...
        for (var key in this.singleInputTickets) {
        ...
        let fieldsClone = JSON.parse(JSON.stringify(this.singleInputTickets));
    ...
    getAttendeesByTypeAndOcode(ocode, ticketTitle):Promise<any[]>{
        ...
        return new Promise((resolve, reject)=>{
            this.getAttendeesByOcode(ocode).then((attendees)=>{
                ...
                let totalPromotedAttendees:any[] = [];
                for(let attendee of this.allAttendeesWithOcode){
                    if(attendee.ticketTitle == ticketTitle){
