...
@Component({
    selector: 'app-my-event-attendees',
    template: `
            ...
            <tbody>
                <tr *ngFor="let attendee of attendeeList;let i = index">
                    <td>
                        ...
                        <a (click)="triggerEmail(attendee.email, attendee.firstName, attendee.lastName)" class="pull-right">
...
export class MyEventAttendeesComponent implements OnInit {
    ...
    createFullData() {
        ...
        for (const attendee of this.attendeeList) {
            const entry: any = {};
        ...
        const jsonAction = function (err, csv) {
            // tslint:disable-next-line:curly
            if (err) throw err;

            const escapedCSV = encodeURI(csv);
