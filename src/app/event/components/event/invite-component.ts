        ...
        <div>
            ...
            <div class="input-group">
                ...
                <span class="input-group-btn">
                        <button id="copy-link" data-clipboard-target="#link" class="btn btn-primary"
                            (click)="copy()"><i class="fa fa-files-o"></i> Copy to Clipboard</button>
                </span>
            </div>
...
export class InviteComponent implements OnInit, AfterViewChecked {
    ...
    @Input() imagePath;
    @Input() width;
    @Input() hideSocials = false;
    @Input() twoSocials = false;
    @Input() hideButtonText = false;
    ...
    link: string;
    viewChecked = false;
    popupSettings = 'toolbar=0,status=0,resizable=yes,width=450,height=600';
    ...
    shareTwitter() {
        ...
        const text = encodeURIComponent(`Join my event at ōEvent! | ${this.title}`);
        const url = `https://www.twitter.com/intent/tweet?text=${text}&url=${this.link}`;
    ...
    shareFacebook() {
        ...
        const url = `https://www.facebook.com/sharer/sharer.php?u=${this.link}`;
    ...
    shareReddit() {
        const title = encodeURIComponent(`Join my event at ōEvent! | ${this.title}`);
        const url = `https://www.reddit.com/submit?url=${this.link}&title=${title}`;
        window.open(
            url,
            'REDDIT',
            this.popupSettings
    ...
    shareGoogle() {
        const url = `https://plus.google.com/share?url=${this.link}`;
        window.open(
            url,
            'Google +',
            this.popupSettings
    ...
    shareEmail() {
        const subject = encodeURIComponent(`Join my event at ōEvent! | ${this.title}`);
        const body = encodeURIComponent(`Sign up with oevent for , ${this.title} at ${this.link}`);
